...
@Component({
  ...
  standalone: true,
  imports: [DatePipe, LucideAngularModule, NgIf],
...
export class DetailsCertificateComponent {
  ...
  downloadCertificate() {
    this.certificateService.downloadKeyStore(this.certificateId).subscribe({
      next: (blob: Blob) => {
  ...
  isDownloadEnable() {
    return this.certificate.status === CertificateStatus.Active;
  }
}
